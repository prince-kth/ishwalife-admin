...
# Function to calculate D4 (Drekkana) chart position
def calculate_d4(total_degrees):
    base_rashi = int(total_degrees / 30)
    degree_in_rashi = total_degrees % 30
    drekkana = int(degree_in_rashi / 10)
    
    # Calculate D4 rashi based on the drekkana
    rashi_increment = drekkana * 4
    final_rashi_num = (base_rashi * 4 + rashi_increment) % 12
...
def calculate_d10(total_degrees):
    ...
    final_rashi_num = (base_rashi * 10 + division) % 12
...
def calculate_d60(total_degrees):
    base_rashi = int(total_degrees / 30)
    degree_in_rashi = total_degrees % 30
    division = int(degree_in_rashi / 0.5)  # Each division is 0.5 degrees
    
    final_rashi_num = (base_rashi * 60 + division) % 12
...
def calculate_extended_planetary_info(julian_day, lat, lon):
    ...
    for planet, planet_num in planet_mappings:
        ...
        divisional_charts = {
            ...
            'D60': calculate_d60(total_degrees)
        }
