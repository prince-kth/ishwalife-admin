...
def calculate_d60(total_degrees, planet=None):
    ...
    Traditional method based on Brihat Parashara Hora Shastra
    Each rashi (30°) is divided into 60 parts of 0.5° each
    """
    base_rashi = int(total_degrees / 30)  # 0-11
    degree_in_rashi = total_degrees % 30
    
    # Calculate navamsa position first (D9)
    nav_division = int(degree_in_rashi / 3.333333)
    
    # Calculate remaining degrees after navamsa
    remaining_degrees = degree_in_rashi - (nav_division * 3.333333)
    
    # Calculate D60 division within navamsa (each 0.5° = one sign movement)
    d60_division = int(remaining_degrees / 0.5)

    # Set starting rashi based on planet and sign type
    is_odd_rashi = (base_rashi % 2 == 0)  # Note: 0-based index, so even index = odd rashi
    
    if planet == 'Sun':
        # Sun starts from Leo (4) in odd signs, Sagittarius (8) in even signs
        start_rashi = 4 if is_odd_rashi else 8
    elif planet == 'Mercury':
        # Mercury starts from Virgo (5) in odd signs, Pisces (11) in even signs
        start_rashi = 5 if is_odd_rashi else 11
    else:
        # Other planets: start from own sign if odd, 9th from it if even
        if is_odd_rashi:
            start_rashi = base_rashi
        else:
    ...
    # First move by navamsa
    nav_pos = (start_rashi + nav_division) % 12
    # Then move by D60 divisions
    final_rashi_num = (nav_pos + d60_division) % 12
