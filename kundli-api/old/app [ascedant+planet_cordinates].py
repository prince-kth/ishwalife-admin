...
def calculate_house_positions(jd, lat, lon):
    """Calculate house positions using the same method as first file"""
...
def calculate_extended_planetary_info(julian_day, lat, lon):
    ...
    # Set ayanamsa to Lahiri (same as first file)
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    ayanamsa = swe.get_ayanamsa(julian_day)
    
    # Calculate houses and ascendant using the method from first file
    houses, ascendant = calculate_house_positions(julian_day, lat, lon)
    
    # Adjust for ayanamsa (same as first file)
    ...
    for planet in ['Sun', 'Moon', 'Mars', 'Mercury', 'Venus', 'Jupiter', 'Saturn']:
        ...
        longitude = (planet_info[0][0] - ayanamsa) % 360  # Adjust for ayanamsa
    ...
    # Calculate Rahu and Ketu with ayanamsa adjustment
    ...
    # Include Ascendant using the corrected calculation from first file
